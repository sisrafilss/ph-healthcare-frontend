...
export const doctorsColumns: Column<IDoctor>[] = [
  {
    header: 'Doctor',
    accessor: (doctor) => (
      <UserInfoCell name={doctor.name} email={doctor.email} photo={doctor.profilePhoto} />
    ),
  },
  {
    header: 'Specialties',
  ...
  },
  {
    header: 'Experience',
    accessor: (doctor) => (
      <span className="text-sm font-medium">{doctor.experience ?? 0} years</span>
    ),
  },
  {
    header: 'Fee',
    accessor: (doctor) => (
      <span className="text-sm font-semibold text-green-600">${doctor.appointmentFee}</span>
    ),
  },
  {
    header: 'Rating',
